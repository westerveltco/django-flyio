...
1. Create a new git branch off of `main` for the release.
   ...
   Prefer the convention `release-<version>`, where `<version>` is the next incremental version number (e.g. `release-v0.1.1` for version 0.1.1).
...
6. Draft a [new release](https://github.com/westerveltco/django-flyio/releases/new) on GitHub.

   Use the version number with a leading `v` as the tag name (e.g. `v0.1.1`).
